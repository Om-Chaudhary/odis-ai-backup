...
const TEST_SCENARIOS: Record<TestScenario, { label: string; data: DynamicVariables }> = {
  'discharge-wellness': {
  ...
  'followup-ear': {
    ...
    data: {
      ...
      callType: 'follow-up',
      condition: 'bilateral bacterial otitis externa, which means an ear infection in both ears',
      clinicPhone: 'four zero eight, two five nine, eight seven six five',
      emergencyPhone: 'four zero eight, eight six five, four three two one',
      ...
      recheckDate: 'November twenty second so we can reexamine his ears and make sure the infection has completely cleared',
    },
  },
  'followup-orthopedic': {
    ...
    data: {
      ...
      callType: 'follow-up',
      condition: 'osteoarthritis in the right hip joint causing intermittent lameness',
      clinicPhone: 'four zero eight, two five nine, eight seven six five',
      emergencyPhone: 'four zero eight, eight six five, four three two one',
...
export function VapiTestPage() {
  ...
  const [selectedScenario, setSelectedScenario] = useState<TestScenario>('discharge-wellness');
  const [variables, setVariables] = useState<DynamicVariables>(TEST_SCENARIOS['discharge-wellness'].data);
          ...
          if (msg.type === "conversation-update") {
            ...
            setTranscript((prev) => [
              ...prev,
              {
                ...
                text: (msg.transcript as string) ?? (msg.transcriptType as string) ?? "",
              },
            ]);
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.error("Microphone access is required for voice calls. Please allow microphone access and try again.");
        setIsConnecting(false);
        return;
      }
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900">Vapi Assistant Test Page</h1>
        ...
        <Card className="bg-white shadow-lg border-slate-200">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="scenario" className="text-slate-700 font-medium">Test Scenario</Label>
              <Select
                value={selectedScenario}
                onValueChange={(value) => handleScenarioChange(value as TestScenario)}
              ...
              >
                ...
                <SelectContent>
                  {(Object.entries(TEST_SCENARIOS) as [TestScenario, typeof TEST_SCENARIOS[TestScenario]][]).map(([key, { label }]) => (
              ...
              <p className="text-xs text-slate-500">
                Select a preset scenario or choose "Custom" to manually configure all fields
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="clinicName" className="text-slate-700 font-medium">
                  Clinic Name <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="clinicName"
                  value={variables.clinicName}
                  onChange={(e) => handleVariableChange("clinicName", e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="border-slate-300 placeholder:text-slate-400"
                />
                <p className="text-xs text-slate-500">First name only (e.g., "Sarah", not "Dr. Sarah")</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="petName" className="text-slate-700 font-medium">
                  Pet Name <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="petName"
                  value={variables.petName}
                  onChange={(e) => handleVariableChange("petName", e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleVariableChange("ownerName", e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleVariableChange("appointmentDate", e.target.value)}
                  placeholder="e.g., November eighth (spelled out)"
                  disabled={isConnected || isConnecting}
                  className="border-slate-300 placeholder:text-slate-400"
                />
                <p className="text-xs text-slate-500">Spell out the date for natural speech (e.g., "November eighth")</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="callType" className="text-slate-700 font-medium">
                  Call Type <span className="text-red-500">*</span>
                </Label>
                <Select
                  value={variables.callType}
                  onValueChange={(value) => handleVariableChange("callType", value as 'discharge' | 'follow-up')}
                  disabled={isConnected || isConnecting}
                >
                  <SelectTrigger className="border-slate-300">
              ...
              <div className="space-y-2">
                <Label htmlFor="clinicPhone" className="text-slate-700 font-medium">
                  Clinic Phone <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="clinicPhone"
                  value={variables.clinicPhone}
                  onChange={(e) => handleVariableChange("clinicPhone", e.target.value)}
                  placeholder="e.g., five five five, two three four, five six seven eight"
                  disabled={isConnected || isConnecting}
                  className="border-slate-300 placeholder:text-slate-400"
                />
                <p className="text-xs text-slate-500">Spelled out for natural speech</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="emergencyPhone" className="text-slate-700 font-medium">
                  Emergency Phone <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="emergencyPhone"
                  value={variables.emergencyPhone}
                  onChange={(e) => handleVariableChange("emergencyPhone", e.target.value)}
              ...
              <div className="space-y-2">
                <Label htmlFor="dischargeSummary" className="text-slate-700 font-medium">
                  Discharge Summary <span className="text-red-500">*</span>
                </Label>
                <Textarea
                  id="dischargeSummary"
                  value={variables.dischargeSummary}
                  onChange={(e) => handleVariableChange("dischargeSummary", e.target.value)}
            ...
            {variables.callType === 'discharge' && (
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="subType" className="text-slate-700 font-medium">
                      Discharge Sub-Type
                    </Label>
                    <Select
                      value={variables.subType ?? ''}
                      onValueChange={(value) => handleVariableChange("subType", value as 'wellness' | 'vaccination')}
                      disabled={isConnected || isConnecting}
                    >
                      <SelectTrigger className="border-slate-300">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="nextSteps" className="text-slate-700 font-medium">
                      Next Steps
                    </Label>
                    <Textarea
                      id="nextSteps"
                      value={variables.nextSteps ?? ''}
                      onChange={(e) => handleVariableChange("nextSteps", e.target.value)}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleVariableChange("condition", e.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => handleVariableChange("medications", e.target.value)}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="recheckDate" className="text-slate-700 font-medium">
                      Recheck Date
                    </Label>
                    <Input
                      id="recheckDate"
                      value={variables.recheckDate ?? ''}
                      onChange={(e) => handleVariableChange("recheckDate", e.target.value)}
        ...
        <div className="space-y-6">
          <Card className="bg-white shadow-lg border-slate-200">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600">Current call connection status</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-white shadow-lg border-slate-200">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600">Real-time conversation transcript</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  transcript.map((item, index) => (
                    <div
                      ...
                      className={`p-3 rounded-lg ${
                        item.role === "assistant"
                          ? "bg-teal-50 ml-4 border border-teal-200"
                          : "bg-slate-100 mr-4 border border-slate-200"
                      }`}
                    >
                      <p className="text-xs font-semibold mb-1 capitalize text-slate-700">
                        {item.role}
                      </p>
            ...
            {showDebug && (
              <CardContent>
                ...
                <p className="text-xs text-slate-500 mt-2">
                  This is the payload structure being sent to the Vapi SDK start() method.
