...
import { DollarSign, Wifi, Copy, AlertCircle } from "lucide-react";
...
// Typewriter component that reveals text based on progress
const TypewriterText = ({
  ...
  const displayText = useTransform(progress, (value) => {
    const charCount = Math.floor(value * text.length);
...
export const GoogleGeminiEffectDemo2 = ({
  pathLengths,
  title,
  description,
  className,
  ...
  // Clunky tools opacity: fade out quickly as animation progresses
  const oldScribeOpacity = useTransform(pathLengths[0]!, [0.2, 0.3], [1, 0]);

  // Individual icon positions and scales for "scattered" effect
  const dollarScale = useTransform(pathLengths[0]!, [0, 0.2], [1, 0.8]);
  const wifiScale = useTransform(pathLengths[0]!, [0, 0.2], [1, 0.8]);
  const copyScale = useTransform(pathLengths[0]!, [0, 0.2], [1, 0.8]);
  const alertScale = useTransform(pathLengths[0]!, [0, 0.2], [1, 0.8]);

  // SOAP card opacity: fade in after old tools fade out
  const cardOpacity = useTransform(pathLengths[0]!, [0.25, 0.35], [0, 1]);

  const opacity1 = useTransform(pathLengths[1]!, [0.3, 0.4], [0, 1]);
  const progress1 = useTransform(pathLengths[1]!, [0.3, 0.4], [0, 1]);

  const opacity2 = useTransform(pathLengths[2]!, [0.35, 0.45], [0, 1]);
  const progress2 = useTransform(pathLengths[2]!, [0.35, 0.45], [0, 1]);

  const opacity3 = useTransform(pathLengths[3]!, [0.4, 0.5], [0, 1]);
  const progress3 = useTransform(pathLengths[3]!, [0.4, 0.5], [0, 1]);

  const opacity4 = useTransform(pathLengths[4]!, [0.45, 0.55], [0, 1]);
  const progress4 = useTransform(pathLengths[4]!, [0.45, 0.55], [0, 1]);

  return (
    <div className={cn("sticky top-80", className)}>
      <p className="font-display bg-gradient-to-b from-slate-800 to-slate-600 bg-clip-text pb-4 text-center text-3xl font-bold text-transparent md:text-7xl">
        {title ?? `Build with Aceternity UI`}
      </p>
      <p className="mx-auto mt-4 max-w-2xl px-4 text-center font-serif text-base font-normal text-slate-600 md:text-xl">
        {description ??
          `Scroll this component and see the bottom SVG come to life wow this
        works!`}
      </p>
      <div className="absolute -top-60 flex h-[890px] w-full items-center justify-center md:-top-40">
        {/* Clunky Old Scribe Tools - scattered and fade out */}
        <motion.div
          className="absolute z-30 mt-32 flex items-center justify-center md:mt-48"
          style={{ opacity: oldScribeOpacity }}
        >
          ...
          <motion.div
            className="absolute -bottom-16 -left-16 flex flex-col items-center gap-1"
          ...
          <motion.div className="flex h-28 w-28 items-center justify-center rounded-3xl border-4 border-dashed border-slate-400 bg-white/80 backdrop-blur-sm md:h-36 md:w-36">
            <span className="text-center font-mono text-xs font-semibold text-slate-600 md:text-sm">
              Old
              <br />
              Scribes
            </span>
        ...
        {/* SOAP Note Card - fades in after old tools fade out */}
        <motion.div
          className="absolute z-30 mx-4 mt-8 max-w-2xl rounded-2xl border-2 border-teal-200 bg-white/90 px-4 py-4 font-mono text-xs shadow-2xl backdrop-blur-sm md:mt-24 md:px-8 md:py-6 md:text-sm"
          style={{ opacity: cardOpacity }}
        >
          <motion.div
            className="mb-3 border-b border-teal-200 pb-2 text-xs font-semibold text-teal-600 md:text-sm"
            style={{ opacity: cardOpacity }}
          >
            SOAP Note Generated - Seamlessly
          </motion.div>
          <div className="space-y-3 text-slate-700">
            <motion.div style={{ opacity: opacity1 }}>
              <span className="font-bold text-teal-700">S:</span>{" "}
              <TypewriterText
                text="Owner reports limping on left hind leg for 2 days..."
                progress={progress1}
              />
            </motion.div>
            <motion.div style={{ opacity: opacity2 }}>
              <span className="font-bold text-teal-700">O:</span>{" "}
              <TypewriterText
                text="T: 101.2°F, HR: 92, RR: 24. Lameness grade 2/5..."
                progress={progress2}
              />
            </motion.div>
            <motion.div style={{ opacity: opacity3 }}>
              ...
              <TypewriterText
                text="Cranial cruciate ligament rupture"
                progress={progress3}
              />
            </motion.div>
            <motion.div style={{ opacity: opacity4 }}>
              ...
              <TypewriterText
                text="TPLO surgery recommended. Carprofen 75mg..."
                progress={progress4}
              />
            </motion.div>
          </div>
