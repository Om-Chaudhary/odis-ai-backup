...
export function validateDynamicVariables(
  ...
  } else if (variables.agentName.includes('Dr.') || variables.agentName.includes('Doctor')) {
    warnings.push(
  ...
  } else if (variables.callType !== 'discharge' && variables.callType !== 'follow-up') {
    errors.push(`callType must be "discharge" or "follow-up", got "${variables.callType}"`);
  }
  ...
  if (variables.callType === 'discharge') {
    if (variables.subType && !['wellness', 'vaccination'].includes(variables.subType)) {
      errors.push(`subType for discharge calls must be "wellness" or "vaccination", got "${variables.subType}"`);
    }

    // Warnings for follow-up fields being present on discharge calls
    if (variables.condition) {
      warnings.push('condition field is not used for discharge calls (only for follow-up calls)');
    }
    if (variables.medications) {
      warnings.push('medications field is not used for discharge calls (only for follow-up calls)');
    }
  }
  ...
  if (variables.callType === 'follow-up') {
    ...
    if (variables.recheckDate && /\d{1,2}\/\d{1,2}\/\d{2,4}/.test(variables.recheckDate)) {
      warnings.push(
    ...
    if (variables.subType) {
      warnings.push('subType field is not used for follow-up calls (only for discharge calls)');
    }
  }
  ...
  if (variables.warningSignsToMonitor && !Array.isArray(variables.warningSignsToMonitor)) {
  ...
  if (variables.emergencyCriteria && !Array.isArray(variables.emergencyCriteria)) {
  ...
  if (variables.petAge !== undefined) {
    ...
    if (isNaN(age) || age < 0 || age > 30) {
      errors.push(`petAge must be a number between 0 and 30, got "${variables.petAge}"`);
    }
  }
  ...
  if (variables.petWeight !== undefined) {
    ...
    if (isNaN(weight) || weight <= 0 || weight > 300) {
      errors.push(`petWeight must be a positive number up to 300 lbs, got "${variables.petWeight}"`);
    }
  }
  ...
  if (variables.petSpecies && !['dog', 'cat', 'other'].includes(variables.petSpecies)) {
    errors.push(`petSpecies must be "dog", "cat", or "other", got "${variables.petSpecies}"`);
  }
  ...
  const sanitizedVariables = valid ? sanitizeVariables(variables as DynamicVariables) : undefined;
 ...
 */
function validateAssessmentQuestion(question: unknown, index: number): string[] {
  const errors: string[] = [];
  ...
  if (q.expectedPositiveResponse && !Array.isArray(q.expectedPositiveResponse)) {
    errors.push(`${prefix}.expectedPositiveResponse must be an array of strings`);
  }
  ...
  if (q.followUpIfConcerning !== undefined && typeof q.followUpIfConcerning !== 'string') {
    errors.push(`${prefix}.followUpIfConcerning must be a string`);
  }

  if (q.priority !== undefined) {
    if (![1, 2, 3, 4, 5].includes(q.priority)) {
      errors.push(`${prefix}.priority must be 1, 2, 3, 4, or 5, got ${q.priority}`);
    }
  }
 ...
 */
export function sanitizeVariables(variables: DynamicVariables): DynamicVariables {
  ...
  if (variables.nextSteps?.trim()) sanitized.nextSteps = variables.nextSteps.trim();
  if (variables.condition?.trim()) sanitized.condition = variables.condition.trim();
  if (variables.conditionCategory) sanitized.conditionCategory = variables.conditionCategory;
  if (variables.medications?.trim()) sanitized.medications = variables.medications.trim();
  if (variables.recheckDate?.trim()) sanitized.recheckDate = variables.recheckDate.trim();

  // Array fields - filter out empty strings and trim
  if (variables.assessmentQuestions && variables.assessmentQuestions.length > 0) {
    sanitized.assessmentQuestions = variables.assessmentQuestions.map((q) => ({
      ...q,
      question: q.question.trim(),
      context: q.context?.trim(),
      expectedPositiveResponse: q.expectedPositiveResponse?.map((r) => r.trim()).filter(Boolean),
      concerningResponses: q.concerningResponses?.map((r) => r.trim()).filter(Boolean),
      followUpIfConcerning: q.followUpIfConcerning?.trim(),
    }));
  }

  if (variables.warningSignsToMonitor && variables.warningSignsToMonitor.length > 0) {
  ...
  if (
    ...
    sanitized.normalPostTreatmentExpectations = variables.normalPostTreatmentExpectations
      .map((s) => s.trim())
      .filter(Boolean);
  }

  if (variables.emergencyCriteria && variables.emergencyCriteria.length > 0) {
    sanitized.emergencyCriteria = variables.emergencyCriteria.map((s) => s.trim()).filter(Boolean);
  }

  if (variables.urgentCriteria && variables.urgentCriteria.length > 0) {
    sanitized.urgentCriteria = variables.urgentCriteria.map((s) => s.trim()).filter(Boolean);
  }
  ...
  if (variables.petWeight !== undefined) sanitized.petWeight = variables.petWeight;
  if (variables.daysSinceTreatment !== undefined)
 ...
 */
export function inferConditionCategory(conditionString: string): ConditionCategory {
  ...
  if (/surg|spay|neuter|incision|stitches|sutures|castration|ovariohysterectomy/.test(normalized)) {
    return 'post-surgical';
  }

  // Dermatological keywords
  if (/skin|rash|itch|hot spot|derma|pyoderma|allergy|allergic|scratch/.test(normalized)) {
    return 'dermatological';
  }

  // Respiratory keywords
  if (/cough|sneeze|respiratory|wheeze|breathing|pneumonia|bronchitis|kennel cough/.test(normalized)) {
    return 'respiratory';
  }

  // Urinary keywords
  if (/urin|bladder|uti|kidney|renal|crystals|stones|incontinence/.test(normalized)) {
    return 'urinary';
  }
  ...
  if (/seizure|neurological|neuro|paralysis|tremor|ataxia|disc|ivdd/.test(normalized)) {
    return 'neurological';
  }

  // Ophthalmic keywords
  if (/eye|ophthalm|vision|cornea|conjunctivitis|cataract|glaucoma/.test(normalized)) {
    return 'ophthalmic';
  }
  ...
  // Endocrine keywords
  if (/diabetes|diabetic|thyroid|cushing|addison|endocrine|hormone/.test(normalized)) {
    return 'endocrine';
  }
  ...
  // Wound care keywords
  if (/wound|laceration|abscess|bite|puncture|cut|injury|trauma/.test(normalized)) {
    return 'wound-care';
  }

  // Behavioral keywords
  if (/behavior|anxiety|aggression|fear|stress|separation|compulsive/.test(normalized)) {
    return 'behavioral';
  }
 ...
 */
export function hasRequiredFieldsForCallType(variables: Partial<DynamicVariables>): boolean {
  const coreFields = [
  ...
  const hasCoreFields = coreFields.every((field) => field && String(field).trim().length > 0);
