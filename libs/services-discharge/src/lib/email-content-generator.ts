 ...
 *
 * Shared utility for generating formatted discharge email content
 * using the DischargeEmailTemplate.
 *
 * This is extracted from the orchestrator to be reused in the outbound
 * dashboard procedures (approve.ts, schedule-remaining.ts).
 */

import type { StructuredDischargeSummary } from "@odis-ai/validators/discharge-summary";
import type { ClinicBranding } from "@odis-ai/types/clinic-branding";
 ...
 *
 * @param dischargeSummary - Plaintext discharge summary content
 * @param patientName - Patient name
 * @param species - Patient species (optional)
 * @param breed - Patient breed (optional)
 * @param branding - Clinic branding configuration
 * @param structuredContent - Structured discharge summary with medications, home care, etc. (optional)
 * @param visitDate - Visit date (optional, not currently used in template)
 * @returns Email subject, HTML content, and plain text content
 */
export async function generateDischargeEmailContent(
