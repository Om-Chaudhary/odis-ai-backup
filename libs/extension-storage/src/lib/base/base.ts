...
export const createStorage = <D = string>(
  ...
  if (
    ...
    chrome?.storage[storageEnum]
      ...
      .catch((error) => {
        // Use plain console here to avoid cross-package imports from storage base

        console.error("Failed to set storage access level", { error });

        console.warn(
          "Please call .setAccessLevel() in a different context, like a background script.",
        );
      });
  ...
  const _updateFromStorageOnChanged = (
    ...
    void updateCache(valueOrUpdate, cache).then((newCache) => {
      cache = newCache;
      _emitChange();
    });
  };
