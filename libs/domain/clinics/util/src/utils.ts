...
export function buildClinicScopeFilter(
  ...
  // Include clinic_id match if we have a clinic
  if (clinicId) {
    parts.push(`clinic_id.eq.${clinicId}`);
  }

  // Include user_id match for backward compatibility (iOS app, legacy records)
  if (clinicUserIds.length > 0) {
    parts.push(`user_id.in.(${clinicUserIds.join(",")})`);
  }

  // Return combined filter or fallback
  if (parts.length > 0) {
    return parts.join(",");
  }

  // Fallback: shouldn't happen in practice, but prevent matching all records
...
export function buildClinicScopeFilterWithOptions(
  ...
  // Always include clinic_id match
  parts.push(`clinic_id.eq.${clinicId}`);

  // Include user_id match for backward compatibility
  if (clinicUserIds.length > 0) {
    parts.push(`user_id.in.(${clinicUserIds.join(",")})`);
  }
