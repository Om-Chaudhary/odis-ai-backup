...
import { AlertCircle, ArrowRight, PhoneIncoming } from "lucide-react";
    ...
    <div className="mx-auto max-w-2xl px-8 py-12">
      <div className="rounded-lg border border-amber-200/60 bg-amber-50/50 p-8 text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-amber-100">
          <PhoneIncoming className="h-6 w-6 text-amber-600" />
        </div>
        ...
        <div className="mb-6 flex items-start justify-center gap-2 text-amber-700">
          ...
          <p className="text-sm">
            VAPI inbound call configuration has been moved to the clinic
        ...
        {clinicId ? (
          <Button asChild className="bg-amber-600 hover:bg-amber-700">
            <Link
              href={`/dashboard/${clinicSlug}/admin/clinics/${clinicId}?tab=vapi`}
            >
              Go to VAPI Settings
