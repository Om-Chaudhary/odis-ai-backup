  ...
  try {
    ...
    try {
      ...
      if (isNewClient) {
        ...
        logger.debug("Create appointment for existing patient result", {
          result,
        });
      }
...
async function handleRescheduleAppointment(
  ...
  if (
    ...
    res.status(400).json({
      success: false,
      error:
        "cancelAppointmentId, patientId, date, startTime, and reason are required",
      timestamp: new Date().toISOString(),
    });
    return;
  }
