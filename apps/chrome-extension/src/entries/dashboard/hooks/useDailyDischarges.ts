...
export const useDailyDischarges = () => {
  ...
  const hydrateMissingData = async (currentCases: DashboardCase[]) => {
    const casesToHydrate = currentCases.filter((c) => {
      ...
      const isIdexx =
        c.source === "idexx_neo" ||
        !!(meta?.idexx as Record<string, unknown>)?.client_id;
    ...
    setCases((prev) =>
      prev.map((c) =>
        casesToHydrate.some((h) => h.id === c.id)
          ? { ...c, hydrated: { ...c.hydrated!, isLoading: true } }
          : c,
      ),
    );
    ...
    for (const caseItem of casesToHydrate) {
      ...
      const clientId = (meta?.idexx as Record<string, unknown>)?.client_id as
        | string
        | undefined;
      ...
      try {
        ...
        const phone =
          contactResult.phone || caseItem.hydrated?.ownerPhone || null;
        const email =
          contactResult.email || caseItem.hydrated?.ownerEmail || null;
        ...
        odisLogger.error(`Failed to hydrate case ${caseItem.id}`, {
          error: err,
        });
        setCases((prev) =>
          prev.map((c) =>
