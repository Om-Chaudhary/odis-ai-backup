import { SyncFutureAppointmentsButton } from "./schedule/SyncFutureAppointmentsButton";
import { SyncScheduleButton } from "./schedule/SyncScheduleButton";
import { getAuthSession, logger, trackEvent } from "@odis-ai/extension/shared";
...
export const PopupMenuBar = () => {
  ...
  const openDashboard = async () => {
    ...
    try {
      ...
      try {
        ...
        logger.warn("Failed to track dashboard open event", {
          error: trackError,
        });
      }
      ...
      try {
        ...
        logger.warn("Failed to track dashboard open error", {
          error: trackError,
        });
      }
  ...
  const isOnSchedulePage =
    currentTab && isIdexxSchedulePage(currentTab.url, currentTab.title);
  ...
  return (
    <>
      ...
      {/* Show sync buttons when on IDEXX schedule page */}
      {isOnSchedulePage && (
        <>
          <SyncScheduleButton />
          <SyncFutureAppointmentsButton />
        </>
      )}
    </>
  );
