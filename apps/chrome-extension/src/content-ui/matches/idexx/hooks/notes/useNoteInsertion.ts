...
import {
  extractVitalsFromNotes,
  autoFillVitalSigns,
} from "../../utils/extraction/vitals-autofill";
...
export const useNoteInsertion = (ckeditorInfo: CKEditorInfo) => {
  ...
  const insertNote = async (selectedPatient: PatientWithCase | null) => {
    ...
    try {
      ...
      if (!soapNote) {
        alert(
          `No SOAP notes found for ${selectedPatient.name}. Please create a SOAP note first.`,
        );
        return;
      }
      ...
      try {
        ...
        const result = await extractVitalsFromNotes(
          plainTextContent,
          undefined,
        ...
        odisLogger.warn("Failed to extract vitals from note", {
          error: vitalError,
        });
      }
    } catch (error) {
      ...
      if (error instanceof Error && error.message.includes("not signed in")) {
        alert(
          "ODIS Extension: Please sign in to the extension to insert SOAP notes.",
        );
      } else {
        alert(
          `ODIS Extension: Error loading SOAP note: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    } finally {
