...
const createSyncRecord = async (
  ...
  if (error) {
    ...
    if (error.code === "23505") {
      ...
      if (existingError) {
        throw new Error(
          `Failed to fetch existing sync record: ${existingError.message}`,
        );
      }
...
const updateSyncRecord = async (
  ...
  if (updates.syncedCount !== undefined)
    updateData.synced_count = updates.syncedCount;
