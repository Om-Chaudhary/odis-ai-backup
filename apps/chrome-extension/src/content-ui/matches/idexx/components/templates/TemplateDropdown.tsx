...
export const TemplateDropdown = ({
  ...
  useEffect(() => {
    ...
    const notesSection = ckeditorInfo.editorElement.closest(
      ".cke",
    ) as HTMLElement;
    if (!notesSection) {
      return;
  ...
  const handleTemplateSelect = async (template: SOAPTemplate) => {
    try {
      ...
      alert(
        "Failed to insert template. Please ensure the editor is in edit mode.",
      );
    } finally {
  ...
  const filteredTemplatesByCategory = Object.entries(
    templatesByCategory,
  ).reduce(
