...
} from "../../utils/formatting/phone-formatter";
 ...
 */
const ScheduleDischargeCallButton = ({
  menuStyle = false,
}: ScheduleDischargeCallButtonProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [consultationData, setConsultationData] =
    useState<IdexxConsultationPageData | null>(null);
  ...
  const waitForPhoneNumber = async (
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      const phoneNumber =
        data.client.phone ||
        data.client.mobilePhone ||
        data.client.homePhone ||
        data.client.workPhone ||
        "";
      let detectedPhoneType: "mobile" | "home" | "work" | "other" | null = null;

      if (phoneNumber) {
        if (data.client.mobilePhone === phoneNumber)
          detectedPhoneType = "mobile";
        else if (data.client.homePhone === phoneNumber)
          detectedPhoneType = "home";
        else if (data.client.workPhone === phoneNumber)
          detectedPhoneType = "work";
        else detectedPhoneType = "other";

        return {
          phone: normalizePhoneNumber(phoneNumber),
          type: detectedPhoneType,
        };
      }
      ...
      if (data.client.id) {
        try {
          const clientPhoneResult = await fetchClientPhoneNumber(
            data.client.id,
          );
          ...
          odisLogger.error(
            `❌ Failed to fetch phone (attempt ${attempt + 1})`,
  ...
  const handleOpenModal = async () => {
    ...
    try {
      ...
      if (!data) {
        throw new Error(
          "Failed to fetch consultation data. Make sure you are on a consultation page.",
        );
      }
      ...
      if (!validation.valid) {
        throw new Error(
          `Invalid consultation data: ${validation.errors.join(", ")}`,
        );
      }
      ...
      const { phone: phoneNumber, type: detectedPhoneType } =
        await waitForPhoneNumber(data);
      ...
      if (hasAllFields) {
        ...
        const callRequest = transformConsultationToCallRequest(
          updatedData,
          scheduleDate,
          notes,
        );
        ...
        if (!phoneNumber) {
          setWarnings((prev) => [
            ...prev,
            "Phone number not found - please enter manually",
          ]);
        }

        setCustomNotes(
          `Consultation #${data.consultation.id} - ${data.consultation.reason}`,
        );
      }
    } catch (err) {
      ...
      {menuStyle ? (
        ...
        >
          <div className="odis-btn-content">
            <span className="odis-btn-text">
              {isLoading ? "Scheduling..." : "Schedule Discharge Call"}
            </span>
          </div>
        </button>
      ) : (
                ...
                >
                  ...
                  <div style={{ marginBottom: "20px" }}>
                    ...
                    {ownerPhone && !isValidE164PhoneNumber(ownerPhone) && (
                      ...
                      >
                        ⚠️ Phone must be 10 digits (will auto-format to
                        +1XXXXXXXXXX)
                      </p>
                    )}
                    {ownerPhone && isValidE164PhoneNumber(ownerPhone) && (
                  ...
                  <div style={{ marginBottom: "20px" }}>
                    ...
                    <input
                      ...
                      type="text"
                      value={
                        consultationData.pageData.providers[0]?.name ||
                        "Unknown"
                      }
                      readOnly
                      style={{
                  ...
                  <div style={{ marginBottom: "20px" }}>
                    ...
                    >
                      Default: 2 days from now at 2:00 PM (calls auto-adjusted
                      to 12pm-5pm)
                    </p>
                  </div>
