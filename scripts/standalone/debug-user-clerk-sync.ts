/**
 * Debug user Clerk sync status (Standalone version)
 *
...
async function main() {
  ...
  if (!email) {
    ...
    console.log(
      "Usage: pnpm tsx scripts/standalone/debug-user-clerk-sync.ts <email>",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (error || !supabaseUser) {
      ...
      console.log(`   clerk_user_id: ${clerkUser.id} not found in users table`);
      console.log(
      ...
      if (emailUser) {
        ...
        console.log(
          `      Run: pnpm tsx scripts/standalone/link-clerk-account.ts ${email}\n`,
        );
      } else {
        console.log(`   ✅ Solution: Manually create the user record:`);
        console.log(
          `      Run: pnpm tsx scripts/standalone/sync-clerk-user.ts ${email}\n`,
        );
      }
    } else {
      ...
      if (!clinicAccess || clinicAccess.length === 0) {
        ...
        console.log(`✅ User has access to ${clinicAccess.length} clinic(s):`);
