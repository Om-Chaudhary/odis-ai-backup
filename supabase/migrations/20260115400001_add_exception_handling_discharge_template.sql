-- Migration: Add exception handling to discharge template trigger
-- Date: 2026-01-15
-- Purpose: Add defensive exception handling to allow signup even if template creation fails
--
-- This ensures consistency with the SOAP template trigger and improves reliability
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RAISE WARNING 'Failed to create default discharge template for user %: % (SQLSTATE: %)',
...
-- Add comment explaining the exception handling
COMMENT ON FUNCTION public.create_default_discharge_template_for_new_user() IS
'Creates a default discharge template for new users. Includes exception handling to allow signup even if template creation fails.';
